...
CREATE TABLE TRANSAKSI_SHIPPED (no_invoice CHAR(10) NOT NULL, tanggal DATE NOT NULL, waktu_bayar TIMESTAMP, status SMALLINT NOT NULL, total_bayar NUMERIC(10, 2) NOT NULL, email_pembeli VARCHAR(50) NOT NULL, nama_toko VARCHAR(100) NOT NULL, alamat_kirim TEXT NOT NULL, biaya_kirim NUMERIC(10, 2) NOT NULL, no_resi CHAR(16), nama_jasa_kirim VARCHAR(16) NOT NULL, PRIMARY KEY (no_invoice), FOREIGN KEY (email_pembeli) REFERENCES PELANGGAN(email) ON UPDATE CASCADE ON DELETE RESTRICT, FOREIGN KEY (nama_toko, nama_jasa_kirim) REFERENCES TOKO_JASA_KIRIM(nama_toko, jasa_kirim) CONSTRAINT status_chk CHECK (status IN('1','2','3','4')));

CREATE TABLE TRANSAKSI_PULSA (no_invoice CHAR(10) NOT NULL, tanggal DATE NOT NULL, waktu_bayar TIMESTAMP, status SMALLINT, total_bayar NUMERIC(10,2) NOT NULL, email_pembeli VARCHAR(50) NOT NULL, nominal INT NOT NULL, nomor VARCHAR(20) NOT NULL, kode_produk CHAR(8) NOT NULL, PRIMARY KEY (no_invoice), FOREIGN KEY (email_pembeli) REFERENCES PELANGGAN(email), FOREIGN KEY (kode_produk) REFERENCES PRODUK_PULSA(kode_produk), CONSTRAINT status_pulsa_chk CHECK (status IN ('1','2'))
);
...
CREATE TABLE PROMO_PRODUK (id_promo CHAR(6) NOT NULL, kode_produk CHAR(8) NOT NULL, PRIMARY KEY (id_promo, kode_produk), FOREIGN KEY (id_promo) REFERENCES PROMO(id), FOREIGN KEY (kode_produk) REFERENCES PRODUK(kode_produk));
...
--redundant! FK pengirim dan penerima ngarah ke kolom apa?
CREATE TABLE KOMENTAR_DISKUSI (pengirim VARCHAR(50) NOT NULL, penerima VARCHAR(50) NOT NULL, waktu TIMESTAMP NOT NULL, komentar TEXT NOT NULL, PRIMARY KEY (pengirim, penerima, waktu), FOREIGN KEY (pengirim) REFERENCES PELANGGAN(is_penjual), FOREIGN KEY (penerima) REFERENCES PELANGGAN(is_penjual));
